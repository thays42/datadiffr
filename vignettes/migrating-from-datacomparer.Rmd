...
In most scripts, migration means replacing `library(dataCompareR)`
with:
...
orientation. `compare_data()` returns the underlying diff as a
`datadiff_result` (with the diff rows in `$rows`) for programmatic use.
